...
class DMR_env:
    ...
    def Randomize_Start(self, mini_batch_size=10):
        ...
        S0 = torch.where(
            torch.rand(mini_batch_size) < 0.5,
            self.theta_a + 3 * self.inv_vol * torch.randn(mini_batch_size),
            self.theta_b + 3 * self.inv_vol * torch.randn(mini_batch_size)
        )
